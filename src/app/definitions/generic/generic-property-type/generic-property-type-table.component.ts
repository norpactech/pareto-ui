...
import { IGenericPropertyType } from '@app/core/model'
import { ConfirmationDialogComponent } from '@common/dialogs/confirmation-dialog.component'
import { GenericPropertyTypeService } from '@core/service'
...
import { ContextDialogComponent } from './generic-property-type-dialog.component'

@Component({
  selector: 'app-generic-property-type-table',
  templateUrl: './generic-property-type-table.component.html',
  styleUrls: ['./generic-property-type-table.component.scss'],
...
export class GenericPropertyTypeTableComponent implements AfterViewInit {
  ...
  items$!: Observable<IGenericPropertyType[]>
  ...
  hasError = false
  errorText = ''
  selectedRow?: IGenericPropertyType
  ...
  confirmIsActive(isActive: boolean, row: IGenericPropertyType): void {
  ...
  updateIsActive(event: MatCheckboxChange, row: IGenericPropertyType): void {
    ...
    this.GenericPropertyTypeService.deactReact(params).subscribe({
      next: (response) => {
  ...
  showDetail(id: string): void {
    this.GenericPropertyTypeService.get(id).subscribe({
      next: (context: IGenericPropertyType | null) => {
  ...
  ngAfterViewInit() {
    this.items$ = merge(
      ...
      switchMap(() => {
        ...
        return this.GenericPropertyTypeService.find(params)
      }),
      map((results: { total: number; data: IGenericPropertyType[] }) => {
